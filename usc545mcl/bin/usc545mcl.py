...
class Particle(object):
    ...
    def UpdateOdom(self, odom_msg):
        ...
        vx = vx + np.random.normal(0, np.sqrt(LINEAR_MODEL_VAR_X))
        vy = vy + np.random.normal(0, np.sqrt(LINEAR_MODEL_VAR_Y))

        [vx_bar, vy_bar] = RotateBy(np.array([vx, vy]), angle)
        vx_bar = vx_bar * dt + self.map_T_particle.translation[0]
        vy_bar = vy_bar * dt + self.map_T_particle.translation[1]
        self.map_T_particle.translation[0] = vx_bar
        self.map_T_particle.translation[1] = vy_bar

        vth = (odom_msg.twist.twist.angular.z + np.random.normal(0, np.sqrt(ANGULAR_MODEL_VAR))) * dt
        self.map_T_particle.rotation = angle + vth
    ...
    def UpdateScan(self, scan):
        ...
        for i in range(len(scan.ranges)):
            ...
            exp.append(np.exp(A[i],dtype = dtype))
            Z.append(constant_value * exp[i])
...
class ParticleFilter(object):
    ...
    def GetMeanPose(self):
        particle_poses = np.array([[p.map_T_particle.translation[0],
                                    p.map_T_particle.translation[1],
                                    p.map_T_particle.rotation] for p in self.particles])
        normalized_particle_poses = preprocessing.normalize(particle_poses, axis=0)
        clustering = MeanShift(bandwidth=0.002).fit(normalized_particle_poses)
        largest_cluster_index = np.argmax(np.bincount(clustering.labels_))
        largest_cluster_poses = particle_poses[clustering.labels_==largest_cluster_index,:]
        largest_cluster_center = np.mean(largest_cluster_poses, axis=0)
        cluster_transformation = np.array([largest_cluster_center[0], largest_cluster_center[1]])
        return Pose(largest_cluster_center[2],
    ...
    def UpdateScan(self, scan_msg):
        ...
        particle_new = np.random.choice(NUM_PARTICLES,NUM_PARTICLES,p = probabilities)
        particles = []
        for i in range(NUM_PARTICLES):
            particles.append(copy.deepcopy(self.particles[particle_new[i]]))
